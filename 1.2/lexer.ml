...
type streamlined = Ulexing.lexbuf -> Parser.token * 'a as 'a

let rec pad (scanner: streamlined) = function
             [] -> assert false
|       [token] -> token, scanner
| token::tokens -> token, fun _ -> pad scanner tokens 

let mk_cast tokens scanner = pad scanner (Parser.CAST :: List.rev tokens)

let tokenise tokens scanner = pad scanner (List.rev tokens)

let chk_cast tokens scanner buffer =
  let open Parser in
  let token, scanner' = scanner buffer in
  let tokens' = token::tokens
in match token with
     LPAR | CAST | Ident _ -> mk_cast tokens' scanner'
   |                     _ -> tokenise tokens' scanner'

let rec after_LPAR (scanner: streamlined) (buffer: Ulexing.lexbuf) =
  let open Parser in
  let token, scanner' = scanner buffer in
...
in match token with
   ...
   | OBJECT | STRING -> after_kwd tokens' scanner' buffer
   |         Ident _ -> after_id tokens' scanner' buffer
   |               _ -> tokenise tokens' scanner'

and after_id tokens scanner buffer =
  let open Parser in
  let token, scanner' = scanner buffer in
  let tokens' = token::tokens
in match token with
         RPAR -> chk_cast tokens' scanner' buffer
   |    TIMES -> after_times tokens' scanner' buffer
   | LBRACKET -> after_fst_lbr tokens' scanner' buffer
   |      DOT -> after_dot tokens' scanner' buffer
   |        _ -> tokenise tokens' scanner'

and after_kwd tokens scanner buffer =
  let open Parser in
  let token, scanner' = scanner buffer in
  let tokens' = token::tokens
in match token with
         RPAR -> mk_cast tokens' scanner'
   |    TIMES -> after_times tokens' scanner' buffer
   | LBRACKET -> after_fst_lbr tokens' scanner' buffer
   |        _ -> tokenise tokens' scanner'

and after_dot tokens scanner buffer =
  let open Parser in
  let token, scanner' = scanner buffer in
  let tokens' = token::tokens
in match token with
...
and after_times tokens scanner buffer =
  let open Parser in
  let token, scanner' = scanner buffer in
  let tokens' = token::tokens
in match token with
...
and after_fst_lbr tokens scanner buffer =
  let open Parser in
  let token, scanner' = scanner buffer in
  let tokens' = token::tokens
in match token with
        COMMA -> after_fst_lbr tokens' scanner' buffer
   | RBRACKET -> after_more_lbr tokens' scanner' buffer
   |        _ -> tokenise tokens' scanner'

and after_more_lbr tokens scanner buffer =
  let open Parser in
  let token, scanner' = scanner buffer in
  let tokens' = token::tokens
in match token with
...
let repeat t n (scanner: Ulexing.lexbuf -> Parser.token * 'a as 'a) =
  assert (n > 0);
  let rec make n : Parser.token * 'a =
    t, if n = 1 then scanner else fun _ -> make (n-1)
in make n
...
(* val token : Ulexing.lexbuf -> Parser.token * 'a as 'a *)

let rec token = lexer
  newline -> handle_nl lexbuf; token lexbuf
|  blank+ -> update lexbuf; token lexbuf

|     '{' -> update lexbuf; (Parser.LBRACE,     token)
|     '}' -> update lexbuf; (Parser.RBRACE,     token)
|     '[' -> update lexbuf; (Parser.LBRACKET,   token)
|     ']' -> update lexbuf; (Parser.RBRACKET,   token)
|     '(' -> update lexbuf; after_LPAR token lexbuf 
|     ')' -> update lexbuf; (Parser.RPAR,       token)
|     '.' -> update lexbuf; (Parser.DOT,        token)
|     ',' -> update lexbuf; (Parser.COMMA,      token)
|     ':' -> update lexbuf; (Parser.COLON,      token)
|     ';' -> update lexbuf; (Parser.SEMI,       token)
|     '+' -> update lexbuf; (Parser.PLUS,       token)
|     '-' -> update lexbuf; (Parser.MINUS,      token)
|     '*' -> update lexbuf; (Parser.TIMES,      token)
|     '/' -> update lexbuf; (Parser.SLASH,      token)
|     '%' -> update lexbuf; (Parser.PERCENT,    token)
|     '&' -> update lexbuf; (Parser.AMPER,      token)
|     '|' -> update lexbuf; (Parser.MID,        token)
|     '^' -> update lexbuf; (Parser.CIRCUM,     token)
|     '!' -> update lexbuf; (Parser.BANG,       token)
|     '~' -> update lexbuf; (Parser.TILDE,      token)
|     '=' -> update lexbuf; (Parser.ASSIGN,     token)
|     '<' -> update lexbuf; (Parser.LT,         token)
|     '>' -> update lexbuf; (Parser.GT,         token)
|     '?' -> update lexbuf; (Parser.QMARK,      token)
|    "++" -> update lexbuf; (Parser.DPLUS,      token)
|    "--" -> update lexbuf; (Parser.DMINUS,     token)
|    "&&" -> update lexbuf; (Parser.DAMPER,     token)
|    "||" -> update lexbuf; (Parser.DMID,       token)
|    "<<" -> update lexbuf; (Parser.LSHIFT,     token)
|    ">>" -> update lexbuf; (Parser.RSHIFT,     token)
|    "==" -> update lexbuf; (Parser.EQ,         token)
|    "!=" -> update lexbuf; (Parser.NE,         token)
|    "+=" -> update lexbuf; (Parser.PLUSEQ,     token)
|    "-=" -> update lexbuf; (Parser.MINUSEQ,    token)
|    "*=" -> update lexbuf; (Parser.TIMESEQ,    token)
|    "/=" -> update lexbuf; (Parser.DIVEQ,      token)
|    "%=" -> update lexbuf; (Parser.PEREQ,      token)
|    "&=" -> update lexbuf; (Parser.AMPEREQ,    token)
|    "|=" -> update lexbuf; (Parser.MIDEQ,      token)
|    "^=" -> update lexbuf; (Parser.CIREQ,      token)
|   "<<=" -> update lexbuf; (Parser.LSHIFTEQ,   token)
|    "<=" -> update lexbuf; (Parser.LEQ,        token)
|   ">>=" -> update lexbuf; (Parser.RSHIFTEQ,   token)
|    ">=" -> update lexbuf; (Parser.GEQ,        token)
|    "->" -> update lexbuf; (Parser.ARROW,      token)
|     '@' -> after_at lexbuf
|     '"' -> update lexbuf; handle_err (in_norm_str 0 []) lexbuf
|   "@\"" -> update lexbuf; handle_err (in_verb_str 0 []) lexbuf
|    "//" -> update lexbuf; handle_err in_line_com lexbuf;
             token lexbuf 
|    "/*" -> update lexbuf; handle_err in_block_com lexbuf;
             token lexbuf
| integer -> let s = Ulexing.utf8_lexeme lexbuf
             in update lexbuf; (Parser.Int s, token)
|    real -> let s = Ulexing.utf8_lexeme lexbuf
             in update lexbuf; (Parser.Float s, token)
|   ident -> let s = Ulexing.utf8_lexeme lexbuf
             in update lexbuf;
                (try Kwd.find s kwd_map, token with
                   Not_found -> Parser.Ident(Norm,s), token)
|    char -> let s = Ulexing.utf8_lexeme lexbuf
             in update lexbuf; (Parser.Char s, token)
|     eof -> update lexbuf; Parser.EOF, token

| '#' blank* "line" blank* (newline | eof) ->
    update lexbuf; halt "Line indicator expected."
| '#' blank* "line" blank+ ->
    update lexbuf; line_indicator lexbuf; token lexbuf

| _ -> let msg = "Invalid character `"
                 ^ Ulexing.utf8_lexeme lexbuf ^ "'."
       in halt msg

and after_at = lexer
  ident -> let s = Ulexing.utf8_lexeme lexbuf
           in update lexbuf; (Parser.Ident(Verb,s), token)

(* Line indicator (#line) *)

and line_indicator = lexer
  decimal ->
    let n = Ulexing.utf8_lexeme lexbuf
    in update lexbuf;
       end_indicator lexbuf;
       virt_lnum := int_of_string n
| ident ->
    let s = Ulexing.utf8_lexeme lexbuf
    in (match s with
      "default" -> update lexbuf; in_line_com lexbuf;
                   virt_lnum := !lex_curr_p.Lexing.pos_lnum
    |  "hidden" -> update lexbuf; in_line_com lexbuf
    |         _ -> stop ("Invalid line indicator `" ^ s ^ "'.") lexbuf)
| _ -> stop "Line indicator expected." lexbuf

and end_indicator = lexer
  blank* newline -> update lexbuf; handle_nl lexbuf
|    blank* "//" -> update lexbuf; in_line_com lexbuf
|     blank+ '"' -> update lexbuf;
...
and opt_line_com = lexer
  newline -> update lexbuf; handle_nl lexbuf
|     eof -> update lexbuf
|  blank+ -> update lexbuf; opt_line_com lexbuf
|    "//" -> update lexbuf; in_line_com lexbuf
|       _ -> stop "Line comment or newline expected." lexbuf

(* Comments *)

and in_line_com = lexer
  newline -> update lexbuf; handle_nl lexbuf
|     eof -> update lexbuf
|       _ -> update lexbuf; in_line_com lexbuf
...
|       _ -> update lexbuf; in_block_com lexbuf

(* Strings *)

and in_norm_str len acc = lexer
