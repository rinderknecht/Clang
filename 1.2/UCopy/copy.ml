(* Lexer for C# 1.2 for ulex (requires camlp4) *)
...
let copy lexbuf = print_string (Ulexing.utf8_lexeme lexbuf)
...
type streamlined = Ulexing.lexbuf -> Lexis.token * 'a as 'a
...
let mk_cast tokens scanner = pad scanner (Lexis.CAST :: List.rev tokens)
...
(* val token : Ulexing.lexbuf -> Lexis.token * 'a as 'a *)
...
| ident        -> let s = Ulexing.utf8_lexeme lexbuf
                  in update lexbuf; copy lexbuf;
                     (try Kwd.find s kwd_map, token with
                        Not_found -> Lexis.Ident(Norm,s), token)
| '@'          -> after_at lexbuf
...
and after_at = lexer
  ident -> let s = Ulexing.utf8_lexeme lexbuf
           in update lexbuf; copy lexbuf; Lexis.Ident(Verb,s), token
...
let get_token : Ulexing.lexbuf -> Lexis.token =
