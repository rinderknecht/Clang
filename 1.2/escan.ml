...
| eof     -> update lexbuf; Eparser.EOL
| "true"  -> update lexbuf; Eparser.True
| "false" -> update lexbuf; Eparser.False
| '('     -> update lexbuf; Eparser.LPAR
| ')'     -> update lexbuf; Eparser.RPAR
| "||"    -> update lexbuf; Eparser.OR
| "&&"    -> update lexbuf; Eparser.AND
| "=="    -> update lexbuf; Eparser.EQ
| "!="    -> update lexbuf; Eparser.NEQ
| "!"     -> update lexbuf; Eparser.NOT
| "//"    -> update lexbuf;
             handle_err in_line_com lexbuf; token lexbuf
| ident   -> let id = Ulexing.utf8_lexeme lexbuf
             in update lexbuf; Eparser.Ident id
| _       -> let msg = "Invalid character `"
                       ^ Ulexing.utf8_lexeme lexbuf ^ "'."
             in halt msg

and in_line_com = lexer
  newline -> update lexbuf; handle_nl lexbuf; Eparser.EOL
| eof     -> update lexbuf; Eparser.EOL
...
let trace ~(input: filename) =
  match open_in input with
    cin ->
      lex_curr_p := Error.{ !lex_curr_p with pos_fname = input };
      let buffer = Ulexing.from_utf8_channel cin
      and cout = stdout in
      let rec iter () =
